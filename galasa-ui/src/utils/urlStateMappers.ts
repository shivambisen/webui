...
const reverseValueMap: Record<string, string> = Object.fromEntries(
  Object.entries(valueMap).map(([k, v]) => [v, k])
);

// HELPER FUNCTIONS FOR VALUE TRANSFORMATIONS
const minifyListValue = (list: string) =>
  list
    .split(',')
    .map((item) => valueMap[item] || item)
    .join(',');
const expandListValue = (minifiedList: string) =>
  minifiedList
    .split(',')
    .map((item) => reverseValueMap[item] || item)
    .join(',');
