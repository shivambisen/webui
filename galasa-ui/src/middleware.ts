...
const authenticateWithDevToken = async (devToken: string) => {
  ...
  if (devTokenParts.length == 2) {
    ...
    const { jwt } = await authApiClient.postAuthenticate(
      authProperties,
      CLIENT_API_VERSION,
    );
  ...
  try {
    if (
      ...
      if (request.url.includes("/callback")) {
        let responseUrl = request.url.substring(
          0,
          request.url.lastIndexOf("/callback"),
        );

        const shouldReturnToMySettingsPage = cookies().get(
          AuthCookies.SHOULD_REDIRECT_TO_SETTINGS,
        );
...
const handleCallback = async (request: NextRequest, response: NextResponse) => {
  ...
  if (code) {
    ...
    const tokenDescription = request.cookies.get(
      AuthCookies.TOKEN_DESCRIPTION,
    )?.value;
