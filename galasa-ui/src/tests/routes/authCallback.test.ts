...
describe('GET /auth/callback', () => {
  it('stores a JWT in a cookie and redirects to index page', async () => {
    // Given...
    ...
    await AuthCallbackRoute.GET(dummyRequest);

    // Then...
    expect(openIdClientSpy).toHaveBeenCalledTimes(1);
    expect(setCookieSpy).toHaveBeenCalledWith('id_token', 'dummy-jwt');
    expect(deleteCookieSpy).toHaveBeenCalledWith('state');
    expect(redirectSpy).toHaveBeenCalledWith('/');
  });

  it('retries the auth process if a state has not been set', async () => {
    // Given...
